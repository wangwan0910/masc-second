    ...
    def render(self, mode="human", log_dir='./plots'):
        ...
        # for ep in range(num_periods):
            ...
            for i in range(2):
                ...
                ax.plot(range(start, end), [a[i+1][0] for a in self.action_history[start:end]], label=f'Agent {i+1} order', color='mediumslateblue',alpha=0.85, linestyle='--',linewidth=0.8)
                ax.plot(range(start, end), [a[i+1][1] for a in self.action_history[start:end]], label=f'Agent {i+1} price', color="lightsteelblue", alpha=0.4, linewidth=1.7)
                ax.legend(loc='upper left', fontsize='x-small', markerscale=0.5)
                if i == 0:
                    ax.set_ylabel('Demand & order', fontsize='small')
                ax.set_ylim([-1, max(self.factory_capacity, self.retailer_capacity) + 1]) 

            for i in range(2):
                ax = fig.add_subplot(5, 2, i + 3)
                ax.plot(range(start, end), [s[i+1][0] for s in self.state_history[start:end]], label=f'Agent {i+1} Inventory',  color='navy',alpha=0.3, linewidth=2)
                # ax.plot(range(1, len(self.demand_history) + 1), self.demand_history, label=f'Demand', color='gray', linestyle='--', linewidth=0.5)
                ax.plot(range(start, end), [s[i+1][1] for s in self.state_history[start:end]], label=f'Agent {i+1} Backlog', color='b', linewidth=1)
                ax.legend(loc='upper left', fontsize='x-small', markerscale=0.5)
                if i == 0:
                    ax.set_ylabel('Inventory & Backlog', fontsize='small')
                ax.set_ylim([-1, max(self.factory_capacity, self.retailer_capacity) + 1])
            ...
            for i in range(2):
                ...
                ax.plot(range(start, end), [s[i+1][0] for s in self.state_history[start:end]], label=f'Agent {i+1} Inventory', color='navy',alpha=0.3, linewidth=2)
                # ax.plot(range(1, len(self.demand_history) + 1), self.demand_history, label=f'Demand', color='gray', linestyle='--', linewidth=0.5)
                ax.plot(range(start, end), [s[i+1][2] for s in self.state_history[start:end]], label=f'Agent {i+1} Stockout', color='purple', linewidth=1)
                ax.legend(loc='upper left', fontsize='x-small', markerscale=0.5)
                if i == 0:
                    ax.set_ylabel('Inventory & Stockout', fontsize='small')
                ax.set_ylim([-1, max(self.factory_capacity, self.retailer_capacity) + 1])

            for i in range(2):
                ax = fig.add_subplot(5, 2, i + 7)
                ax.plot(range(start, end), [r[i+1] for r in self.reward_history[start:end]], label=f'Agent {i+1} Profit', color="m", alpha=0.8,linewidth=0.65)
                # ax.plot(range(start , end),[a[i+1][2] for a in self.action_history[start:end]], label='Show', color='red', linestyle='--', linewidth=0.5)
                ax.legend(loc='upper left', fontsize='x-small', markerscale=0.5)
                if i == 0:
                    ax.set_ylabel('Profit & price', fontsize='small')
                ax.set_xlabel('Step')
            ...
            for i in range(2):
                ...
                ax.legend(loc='upper left', fontsize='x-small', markerscale=0.5)
                if i == 0:
                    ax.set_ylabel('Show', fontsize='small')
                ax.set_xlabel('Step')
        ...
        filename = f"POV{self.v_f}T{self.task}28.csv"
