...
class POMultiAgent1W1F(MultiAgentEnv): 
    def __init__(self,  *args, **kwargs):#
        ...
        self.observation_space = MultiDiscrete([factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,self.periods+1]) # [inventory,backlog,stockout,Factory_inventory]
    ...
    def reward(self, x, a_retailer,p_retailer,a_factory, p_factory,d, y): 
        ...
        factory_reward = - c * max(min(y[2][0] + a_factory, self.factory_capacity) - y[2][0], 0)-h * y[2][0] -self.b*y[2][1]-self.s[1]*y[2][2]+ p_factory * max(min(x[2][0] + x[2][5], self.factory_capacity) - x[2][0], 0) 
        if self.task == 2:
        ...
        retailer_reward =- p_factory * max(min(y[1][0] + y[2][5], self.retailer_capacity) - y[1][0], 0)-h * y[1][0] -self.b*y[1][1]-self.s[0]*y[1][2]  + p_retailer * max(min(x[1][0] +x[1][5], self.retailer_capacity) - x[1][0], 0) 
    ...
    def step(self, actions): 
        ...
        print(f"Agent 1 - Percentage of 1: {agent1_percentage_1:.2f}%")
        print(f"Agent 2 - Percentage of 1: {agent2_percentage_1:.2f}%")
