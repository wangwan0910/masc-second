...
class POMultiAgent1W1F(MultiAgentEnv): 
    ...
    def transition(self,agent_ids, x, a_factory, a_retailer,V_F,d):  
         ...
         if V_F == 0:
            ...
            IVF = V_F*self.factory_capacity
    
         
    
         x = {1: np.array([I[0],BL[0],SO[0],d_history[0],d_history[1],d_history[2],IVF,self.period]),
    ...
    def step(self, actions): 
        ...
        actions = {i:a for i , a in actions.items()}
        for key in actions:
    ...
    def render(self, mode="human", log_dir='./plots'):
        ...
        if ep_count % p == 0 and len(self.state_history) % p==0:
            ...
            for i in range(2):
                ...
                #else:
                    #ax.plot(range(start, end), [a[i+1][0] for a in self.action_history[start:end]], label=f'Supplier Order', color='mediumslateblue',alpha=0.85, linewidth=0.8){agent_labels[i]} Stockout: {mean_sto:.2f}
            ...
            for i in range(2):
                ...
                ax.plot(range(start, end), [s[i+1][2] for s in self.state_history[start:end]], label=f'', color='purple', linewidth=1)
                ax.legend(loc='upper left', fontsize='medium', markerscale=0.5)
                if i == 0:
            ...
            for i in range(2):
                ...
                ax.plot(range(start , end),[a[i+1][2] for a in self.action_history[start:end]],
                        label=f'Trust: {count_1_percent:.2f}%, Lying: {count_lying_percent:.2f}%, No Coop: {count_0_percent:.2f}%',
