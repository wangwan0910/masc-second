...
class POMultiAgent1W1F(MultiAgentEnv): 
    ...
    def transition(self,agent_ids, x, a_factory, a_retailer,V_F,d): 
        ...
        I[1] = np.clip(I[1],0, self.factory_capacity)
        BL[1] = np.clip(BL[1],0, self.factory_capacity)
        SO[1] = np.clip(SO[1],0, self.factory_capacity)
        ...
        if not factory_stockout:
            ...
            I[0] = np.clip(I[0],0, self.retailer_capacity)
            BL[0] = np.clip(BL[0],0, self.retailer_capacity)
            SO[0] = np.clip(SO[0],0, self.retailer_capacity)
    ...
    def step(self, actions): 
        ...
        # print('--------agent_ids---------',agent_ids)# {1: array([12,  8]), 2: array([8, 6])}
        ...
        # precents = {1:np.array([agent1_percentage_1,agent1_percentage_0]), 2:np.array([agent2_percentage_1,agent2_percentage_0])}
        precents = {
        1: np.array([f"{agent1_percentage_1:.4f}%", f"{agent1_percentage_0:.4f}%"]),
        2: np.array([f"{agent2_percentage_1:.4f}%", f"{agent2_percentage_0:.4f}%"])
    }
        print("precents",precents)
        
        self.precent_history.append(precents.copy())
    ...
    def is_done(self, agent_id): 
        if self.stockoutCount[agent_id] >= self.stockoutCountS[agent_id] and self.day[agent_id] >= self.days[agent_id]:
            print(' self.stockoutCount[agent_id]', self.stockoutCount[agent_id])
        # if (self.stockoutCount>=3 ) or (self.day[agent_id] >= self.days[agent_id]):
            done = True
        else:
            done = False
    ...
    def save_to_csv(self, filename=None):
        ...
        for i in range(len(self.state_history)):
            row = {
                ...
                "action_2_SHOW_Precent": self.precent_history[i][2][0],  ### 0 or 1
