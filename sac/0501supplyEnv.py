        ...
        # for ep in range(num_periods):
            ...
            for i in range(2):
                ...
                if i==0:
                    ...
                    std_price = np.std(prices, ddof=1)  # Using ddof=1 for sample standard deviation

                    #mean_price = sum([a[i+1][1] for a in self.action_history[start:end]]) / (end - start)
                    ax.plot(range(start, end), [a[i+1][1] for a in self.action_history[start:end]], label=f'{agent_labels[i]} Mean Price: {mean_price:.2f}, Std Dev: {std_price:.2f}', color="lightsteelblue", alpha=0.4, linewidth=1.7)
